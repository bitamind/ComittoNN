...
{
//	LOGD("ImageMarginCut : p=%d, h=%d, i=%d, ow=%d, oh=%d, m=%d", Page, Half, Index, OrgWidth, OrgHeight, Margin);
...
//		LOGD("ImageMarginCut : yy=%d", yy);
	...
	if (Margin < 5 && CutT * space / 100 + CutB * space / 100 <= 0) {
		// 0になったら抜ける(0除算もあるし)
//		LOGD("ImageMarginCut : return=0 ct=%d, cb=%d", CutT, CutB);
		return 0;
	}
...
//		LOGD("ImageMarginCut : xx=%d", xx);
	...
	for (int xx = OrgWidth - 1 ; xx >= OrgWidth - CheckCY ; xx --) {
//		LOGD("ImageMarginCut : xx=%d", xx);
		overcnt = 0;	// 白でないカウンタ
		CutR = OrgWidth - 1 - xx;
		for (yy = CutT + 1 ; yy < OrgHeight - CutB ; yy ++) {
	...
	int SclWidth  = OrgWidth - CutCX;
	int SclHeight = OrgHeight - CutCY;

//	LOGD("ImageMarginCut : cl=%d, cr=%d, ct=%d, cb=%d, sw=%d, sh=%d", CutL, CutR, CutT, CutB, SclWidth, SclHeight);
