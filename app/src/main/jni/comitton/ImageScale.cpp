...
int CreateScale(int Page, int Half, int SclWidth, int SclHeight, int algorithm, int Rotate, int Margin, int Bright, int Gamma, int Param)
{
	...
	if (Moire) {
		// 50%以下にする場合は半分に落とす
		while (SclWidth <= OrgWidth / 2 && SclHeight <= OrgHeight / 2) {
			// 50%以下の縮小
			NowWidth  = OrgWidth / 2;
		...
		// 50%以上の縮小がある場合
		int zw = SclWidth * 100 /  OrgWidth;
		int zh = SclHeight * 100 /  OrgHeight;
		if (zw < 100 && zh < 100) {
			// 元データ配列化
			ret = SetLinesPtr(Page, Half, Index, OrgWidth, OrgHeight);
			if (ret < 0) {
...
//			Index ++;
	...
	double scale_x = exp(log((double)SclWidth / (double)OrgWidth) / 2.0);
	double scale_y = exp(log((double)SclHeight / (double)OrgHeight) / 2.0);
...
//		LOGD("ImageScale : loop=1");
	...
	if (Pseland != 0) {
		// 元データ配列化
		ret = SetLinesPtr(Page, Half, Index, SclWidth, SclHeight);
		if (ret < 0) {
			return ret;
		}

		// 90°回転
		ret = ImageRotate(Page, Half, Index, SclWidth, SclHeight, 1);
		if (ret < 0) {
			return ret;
		}
		// 古いワークデータは削除
	...
	if (Sharpen > 0) {
		...
		// シャープ化
		ret = ImageSharpen(Page, Half, Index, SclWidth, SclHeight);
		if (ret < 0) {
			return ret;
		}
		// 古いワークデータは削除
	...
	if (Gray > 0) {
		...
		// グレースケール化
		ret = ImageGray(Page, Half, Index, SclWidth, SclHeight);
		if (ret < 0) {
			return ret;
		}
	...
	if (Coloring > 0) {
		...
		// 自動着色
		ret = ImageColoring(Page, Half, Index, SclWidth, SclHeight);
		if (ret < 0) {
			return ret;
		}
		// 色の変化だけなのでワークデータは作成されない
	...
	if (Invert > 0) {
		...
		// 色の反転
		ret = ImageInvert(Page, Half, Index, SclWidth, SclHeight);
		if (ret < 0) {
			return ret;
		}
	...
	if (Bright != 0 || Gamma != 0) {
		...
		// 明るさの調整
		ret = ImageBright(Page, Half, Index, SclWidth, SclHeight, Bright, Gamma);
		if (ret < 0) {
			return ret;
		}
	...
	pData->SclWidth[Half] = SclWidth;
	pData->SclHeight[Half] = SclHeight;
	return 0;
}
