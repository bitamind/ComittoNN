...
public class ThumbnailLoader {
	...
	public ThumbnailLoader(String uri, String path, Handler handler, long id, ArrayList<FileData> files, int sizeW, int sizeH, int cachenum) {
		...
		// キャッシュフォルダ
		mCachePath = Environment.getExternalStorageDirectory() + "/comitto/";
		try {
	...
	protected void saveThumbnailCache(String pathcode, Bitmap bm) {
		// キャッシュとして保存
		...
		if (file.exists()) {
			...
			try {
				...
				bm.compress(CompressFormat.PNG, 80, cacheSave);
				cacheSave.flush();
				cacheSave.close();
			}
			catch (Exception e) {
	...
	public static void deleteThumbnailCache(int leave) {
		...
		for (File file : files) {
			if (!file.isDirectory()) {
				...
				if (name == null || name.length() <= 6 || name.equals("comitto.cache")) {
					// 削除対象外
					continue;
				}
